...
use crate::codegen::models::{AttributeRep, Content, ContentWithMSFA, EntityTemplate, EntityTemplateContent, Filter, FilterAttributeRep, FilterBody, FilterRep, StringWithNamingConvention};
...
impl TemplateParser {
    pub fn parse_template(template: &str) -> anyhow::Result<EntityTemplate> {
        ...
        for pair in pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner() {
            match pair.as_rule() {
                Rule::entity_content => {
                    let entity_content_pair = pair.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?;
                ...
                _ => bail!("Unexpected rule: {:?} cannot be converted to an entity content", pair)
            }
        }
    ...
    fn parse_filter_attribute_rep(filter_attribute_rep_pair: Pair<Rule>) -> anyhow::Result<FilterAttributeRep> {
        ...
        match filter_attribute_rep_pair.as_rule() {
            Rule::filter_attribute_names_and_types_present => {
                ...
                let prefix = Self::parse_content(inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let attribute_name = Self::parse_string_with_naming_convention(&inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let attribute_name_and_type_separator = Self::parse_content(inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let attribute_type = Self::parse_string_with_naming_convention(&inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let attributes_separator = Self::parse_content(inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let _attribute2_name = &inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?;
                let _attribute2_name_separator = inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?;
                let _attribute2_type = inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?;
                let suffix = Self::parse_content(inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
            ...
            Rule::filter_attribute_names_twice_present => {
                let mut inner_pairs = filter_attribute_rep_pair.into_inner();
                let prefix = Self::parse_content(inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let attribute_name = Self::parse_string_with_naming_convention(&inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let attribute_name_twice_separator = Self::parse_content(inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let _attribute_name = &inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?;
                let attributes_separator = Self::parse_content(inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let _attribute2_name = inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?;
                let _attribute2_name_separator = inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?;
                let _attribute2_name = inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?;
                let suffix = Self::parse_content(inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
            ...
            _ => bail!("Unexpected rule: {:?} cannot be converted to an attribute repitition", filter_attribute_rep_pair)
        }
    ...
    fn parse_filter_rep(filter_rep_pair: Pair<Rule>) -> anyhow::Result<FilterRep> {
        match filter_rep_pair.as_rule() {
            Rule::consecutive_filters => {
                let filter_pair = filter_rep_pair.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?;
            ...
            Rule::separated_filters => {
                ...
                let separator = Self::parse_content(inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
            ...
            _ => bail!("Unexpected rule: {:?} cannot be converted to a filter repetition", filter_rep_pair)
        }
    }
    ...
    fn parse_filter(filter_pair: Pair<Rule>) -> anyhow::Result<Filter> {
        ...
        for filter_body_pair in filter_pair.into_inner() {
            let filter_body = Self::parse_filter_body(filter_body_pair.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
            filter_bodies.push(filter_body);
        }
        Ok(Filter { content: filter_bodies })
    }
    ...
    fn parse_filter_body(filter_body_pair: Pair<Rule>) -> anyhow::Result<FilterBody> {
        match filter_body_pair.as_rule() {
            Rule::content_with_msfa => {
                let content_with_msfa_pair = filter_body_pair.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?;
            ...
            Rule::filter_attribute_rep => {
                let filter_attribute_rep_pair = filter_body_pair.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?;
                let filter_attribute_rep = Self::parse_filter_attribute_rep(filter_attribute_rep_pair)?;
                Ok(FilterBody::FilterAttributeRep(filter_attribute_rep))
            }
            _ => bail!("Unexpected rule: {:?} cannot be converted to a filter body", filter_body_pair)
        }
    }
    
    fn parse_content_with_msfa(content_with_msfa_pair: Pair<Rule>) -> anyhow::Result<ContentWithMSFA> {
    ...
    fn parse_attribute_rep(attribute_pair: Pair<Rule>) -> anyhow::Result<AttributeRep> {
        match attribute_pair.as_rule() {
            Rule::attribute_names_and_types_present => {
                ...
                let prefix = Self::parse_content(inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let attribute_name = Self::parse_string_with_naming_convention(&inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let attribute_name_and_type_separator = Self::parse_content(inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let attribute_type = Self::parse_string_with_naming_convention(&inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let attributes_separator = Self::parse_content(inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let _attribute2_name = &inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?;
                let _attribute2_name_separator = inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?;
            ...
            Rule::attribute_names_present => {
                let mut inner_pairs = attribute_pair.into_inner();
                let prefix = Self::parse_content(inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let attribute_name = Self::parse_string_with_naming_convention(&inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let attributes_separator = Self::parse_content(inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
                let _attribute2_name = inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?;
                let suffix = Self::parse_content(inner_pairs.next().ok_or(anyhow!("{ERROR_MSG}"))?.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?)?;
            ...
            _ => bail!("Unexpected rule: {:?} cannot be converted to an attribute repitition", attribute_pair)
        }
    }
    ...
    fn parse_content(content_pair: Pair<Rule>) -> anyhow::Result<Content> {
        match content_pair.as_rule() {
            Rule::static_content => {
                Ok(Content::Static(content_pair.clone().as_str().to_string()))
            }
            Rule::entity_name => {
                let inner_entity_name = content_pair.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?;
                Ok(Content::EntityName(Self::parse_string_with_naming_convention(&inner_entity_name)?))
            }
            Rule::entity_plural => {
                let inner_entity_plural = content_pair.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?;
                Ok(Content::EntityPlural(Self::parse_string_with_naming_convention(&inner_entity_plural)?))
            }
            Rule::primary_key => {
                let inner_primary_key = content_pair.into_inner().next().ok_or(anyhow!("{ERROR_MSG}"))?;
                Ok(Content::PrimaryKey(Self::parse_string_with_naming_convention(&inner_primary_key)?))
            }
            _ => bail!("Unexpected rule: {:?} cannot be converted to a content", content_pair)
        }
    }

    fn parse_string_with_naming_convention(pair: &Pair<Rule>) -> anyhow::Result<StringWithNamingConvention> {
        match pair.as_rule() {
            ...
            | Rule::filter_attribute_type_kebab_case => Ok(StringWithNamingConvention::KebabCase(pair.as_str().to_string())),
            ...
            | Rule::filter_attribute_type_lower_camel_case => Ok(StringWithNamingConvention::LowerCamelCase(pair.as_str().to_string())),
            ...
            | Rule::filter_attribute_type_upper_camel_case => Ok(StringWithNamingConvention::UpperCamelCase(pair.as_str().to_string())),
            ...
            | Rule::filter_attribute_type_shouty_kebab_case => Ok(StringWithNamingConvention::ShoutyKebabCase(pair.as_str().to_string())),
            ...
            | Rule::filter_attribute_type_shouty_snake_case => Ok(StringWithNamingConvention::ShoutySnakeCase(pair.as_str().to_string())),
            ...
            | Rule::filter_attribute_type_snake_case => Ok(StringWithNamingConvention::SnakeCase(pair.as_str().to_string())),
            ...
            | Rule::filter_attribute_type_title_case => Ok(StringWithNamingConvention::TitleCase(pair.as_str().to_string())),
            ...
            | Rule::filter_attribute_type_train_case => Ok(StringWithNamingConvention::TrainCase(pair.as_str().to_string())),

            _ => bail!("Unexpected rule: {:?} cannot be converted to a string with naming convention", pair)
...
mod tests {
    ...
    fn experiment() {
        ...
        let parsed: crate::codegen::models::EntityTemplate = super::TemplateParser::parse_template(&template).unwrap();
        println!("{:?}", parsed);
    }
}